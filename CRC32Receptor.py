...
def detect_error(bin):
    ...
    print(len(msg))
    if verif == 1:
        presentacion.decode_message(from_array_to_string(payload))

    elif verif == -1:
        presentacion.decode_message(from_array_to_string(payload), "Se detectaron errores, por lo tanto la trama se descarta")
